...
# grch38_mRNA_placenta_counts <- subset(grch38_mRNA_placenta_counts,
#                                       rownames(grch38_mRNA_placenta_counts) %in% protein_coding$gene_id)

# # subset to keep only genes with an entrez id
# grch38_mRNA_placenta_counts <- subset(grch38_mRNA_placenta_counts,
#                                       rownames(grch38_mRNA_placenta_counts) %in% entrez_id$gene_id)
...
# QC_Remove <- c("SCP4060_CDN0JANXX", "SCP4010_CDN0JANXX", "SCP4157_CDN0JANXX")
...
# numSamples <- dim(dplyr::filter(DGElist_mRNA_38$samples, Outcome == "PE"))[1] # (number total PE)
...
This code chunk will plot an MD for each of the samples in a loop.

```{r plot MD all samples}

# for (i in 1:ncol(DGElist_mRNA_38)){
#   
#   plotMD(DGElist_mRNA_38[no_sex_chr,],
#          column = i,
#          main = paste(DGElist_mRNA_38$samples$samplename[i],
#                       DGElist_mRNA_38$samples$Outcome[i],
#                       sep = "_"))
#   abline(h = 0,
#          col = "red",
#          lty = 2,
...
## Sanity plots of known PE genes
...
# use eBayes to share information across genes
...
DGElist_mRNA_38$samples$cohort <- factor(DGElist_mRNA_38$samples$cohort, c("SCOPE", "STOP"))

design_mRNA_sex_PE_cohort <- model.matrix(~0 + sex_pathology + cohort,
                                          data = DGElist_mRNA_38$samples)

colnames(design_mRNA_sex_PE_cohort) <- c("F_Uncomplicated", "M_Uncomplicated", "F_PE", "M_PE", "Cohort_STP")

voom_mRNA_sex_PE_cohort <- voom(DGElist_mRNA_38, design_mRNA_sex_PE_cohort, plot=TRUE)

fit_mRNA_sex_PE_cohort <- lmFit(voom_mRNA_sex_PE_cohort, design_mRNA_sex_PE_cohort)

fit_mRNA_sex_PE_cohort <- eBayes(fit_mRNA_sex_PE_cohort)

summary(decideTests(fit_mRNA_sex_PE_cohort)) # before FDR correction

contrast_mRNA_sex_PE_cohort <- makeContrasts(female_PE = F_PE-F_Uncomplicated,
                                      male_PE = M_PE-M_Uncomplicated,
                                      levels = design_mRNA_sex_PE_cohort)

# fit a linear regression to the contrast questions
fit_mRNA_sex_PE_cohort <- contrasts.fit(fit_mRNA_sex_PE_cohort, contrast_mRNA_sex_PE_cohort)

# perform bayesian adjustment
fit_mRNA_sex_PE_cohort <- eBayes(fit_mRNA_sex_PE_cohort)

# summary table of the sexMode fit
summary_fit_sex_PE_cohort <- summary(decideTests(fit_mRNA_sex_PE_cohort,
                                           adjust.method = "fdr",
                                           p.value = 0.05))

# all DE results for the female_PE comparison
allTable_female_PE_cohort <- topTable(fit_mRNA_sex_PE_cohort,
                             coef = 1,
                             n = Inf,
                             sort = "p")

# only significant (after fdr correction) DE results for the female_PE comparison
topTable_female_PE_cohort <- topTable(fit_mRNA_sex_PE_cohort,
                             coef = 1,
                             n = Inf,
                             sort = "p",
                             p = 0.05,
...
# all DE results for the male_PE comparison
allTable_male_PE_cohort <- topTable(fit_mRNA_sex_PE_cohort,
                             coef = 2,
                             n = Inf,
                             sort = "p")

# only significant (after fdr correction) DE results for the male_PE comparison
topTable_male_PE_cohort <- topTable(fit_mRNA_sex_PE_cohort,
                             coef = 2,
                             n = Inf,
                             sort = "p",
                             p = 0.05,
...
decide <- decideTests(fit_mRNA_sex_PE_cohort)
d2 <- topTable(fit_mRNA_sex_PE_cohort, coef = 2, n = Inf, sort = "p") #[,c(2,6)]
d2$threshold <- 0
d2$threshold[0:nrow(topTable_male_PE_cohort)] <- 1
d2$threshold <- as.factor(d2$threshold)
d3 <- decideTests(fit_mRNA_sex_PE_cohort) %>%
  as.data.frame()
d3 <- d3[2]
d4 <- left_join((tibble::rownames_to_column(d2)),
                       (tibble::rownames_to_column(d3)),
                       by = "rowname")
d4$male_PE <- as.factor(d4$male_PE)
d4$neg.log10FDR <- -log10(d4$adj.P.Val)
...
# set colours for the volcano plot
colour <- c("red", "black", "darkgreen")
FDR_FC_Decide <- decideTests(fit_mRNA_sex_PE_cohort, lfc = 0)[,"male_PE"] %>%
  as.data.frame() %>%
  set_colnames("FDR_FC_Decide") %>%
  tibble::rownames_to_column() %>%
  left_join(., d4, by = "rowname") %>% 
...
volcano <- ggplot(data = FDR_FC_Decide,
                                  aes(x = logFC,
                                      y = -log(adj.P.Val, 10),
                                      label = gene_name,
                                      colour = FDR_FC_Decide)) +
  ...
  theme(axis.title.x = element_text(colour = "black",
        ...
        axis.title.y = element_text(colour = "black",
                                   face = "bold")) +
  geom_text_repel(data = dplyr::filter(FDR_FC_Decide, FDR_FC_Decide == 1),
                  aes(label = gene_name),
                  show.legend = FALSE,
...
MAplot <- ggplot(data = FDR_FC_Decide,
                                  aes(x = AveExpr,
                                      y = logFC,
                                      label = gene_name,
                                      colour = FDR_FC_Decide)) +
  geom_point(alpha = 0.9, size = 2.00) +
  xlab(expression("Average "*Log[2]*" Expression")) + ylab(expression(Log[2]*" Fold Change")) +
  scale_color_manual(values = colour, name = "mRNA\nRegulation\n(PE)",
                         breaks = c("-1", "0", "1"),
  ...
  theme(axis.title.x = element_text(colour = "black",
        ...
        axis.title.y = element_text(colour = "black",
                                   face = "bold")) +
  geom_label_repel(data = dplyr::filter(FDR_FC_Decide, FDR_FC_Decide == 1 & logFC >= 1),
                  aes(label = gene_name),
                  fill = "white",
                  show.legend = FALSE,
                  box.padding = 0.5,
  ...
  geom_label_repel(data = dplyr::filter(FDR_FC_Decide, FDR_FC_Decide == -1 & logFC <= -1),
                  ...
                  box.padding = 0.5,
                  max.overlaps = Inf) +
  geom_label_repel(data = dplyr::filter(FDR_FC_Decide, FDR_FC_Decide == 1 & AveExpr > 10),
                  aes(label = gene_name),
                  fill = "white",
                  show.legend = FALSE,
                  box.padding = 0.5,
...
# Sex Independent Differential Expression Analysis - Preeclampsia
  * Option 1: Factorial formula and no contrast
A nested factorial model allows you to compare groups within each sex. This achieves the aim of treating the two sexes separately.
The `model.matrix` here fits sex (M, F), outcome (PE, control), maternal age (continuous), maternal BMI (continuous) and the sex x outcome interaction.
I will use the output from the interaction to investigate the male and female comparisons independently by asking the question, "For each gene is there a non-zero mean difference in the PE and control the male group, or the female group.". This may not be the best way to ask this question given that there is a different PE risk for both younger and older women compared to women aged between these two groups. [Sibai et al. 2005](https://www.sciencedirect.com/science/article/pii/S0140673605179872?casa_token=z5kbmg1Zl1wAAAAA:hZcSmhrMTGPAIOvQgPTtHPVEiizNMFUduVbQXYXqCOzkIbqJoTkl69cS1ft91KHFqhPayvK83432)
Here we fit a linear model to PE and Age, but given that the risk is bimodal there must be a better way to do this?

```{r differential expression using global t-test p value}

# temp <- DGElist_mRNA_38
# ensure sex is a factor
DGElist_mRNA_38$samples$Sex <- factor(DGElist_mRNA_38$samples$Sex, c("F", "M"))

# design to test DE between PE looking at female and male baring pregnancies seperately
design_independent_sex_PE <- model.matrix(~0 + Sex + Outcome + maternalAge + BMI + Sex:Outcome,
                                               data = DGElist_mRNA_38$samples)
# make the column names a bit easier to read
colnames(design_independent_sex_PE) <- c("Male", "Female", "PE", "maternalAge", "BMI", "male_PE")

# perform the voom transform
voom_independent_sex_PE <- voom(DGElist_mRNA_38, design_independent_sex_PE, plot=TRUE)

fit_independent_sex_PE <- lmFit(voom_independent_sex_PE, 
                                     design_independent_sex_PE)
fit_independent_sex_PE <- eBayes(fit_independent_sex_PE)
# defaults
summary(decideTests(fit_independent_sex_PE))
...
decide <- decideTests(fit_independent_sex_PE,
                    method = "global",
                    adjust.method = "BH",
                    p.value = 0.05)

write_fit <- function(fit,
                      results = NULL, 
                      F.adjust = "none",
                      adjust = "fdr",
                      method = "global", ...) {
...
write_fit <- write_fit(fit = fit_independent_sex_PE,
                       results = decide,
                       method = "global",
                       adjust = "fdr")

write_fit_list <- list(
  male_PE = write_fit %>% dplyr::select(contains("male_PE", ignore.case = FALSE),
                                        starts_with("Genes")),
  female_PE = write_fit %>% dplyr::select(contains("Female_PE", ignore.case = FALSE),
                                          starts_with("Genes")),
  maternalAge = write_fit %>% dplyr::select(contains("maternalAge"),
                                            starts_with("Genes"))
  )

# set the alpha variable (significance)
alpha <- 0.05

write_fit_list %<>% lapply(function(x){
  x %>% set_colnames(c("logFC", "t", "p.value", "p.value.adj", "direction", 
                       "gene_name", "gene_biotype",
                       "entrezid"))%>%
    dplyr::mutate(sig=p.value.adj < alpha)%>%
...
```{r volcano plot male comparison}

# set colours for the volcano plot
colour <- c("red", "darkgreen", "black")

volcano <- ggplot(data = write_fit_list$female_PE,
                                  aes(x = logFC,
                                      y = -log10(p.value.adj),
                                      label = rownames(write_fit_list$female_PE),
                                      colour = as.factor(direction))) +
  geom_point(alpha = 0.9, 
             size = 2.00) +
  xlab(expression(Log[2]*" Fold Change")) +
  ylab(expression("-ve "*Log[10]*" FDR")) +
  ggtitle("Female comparison") +
  ...
  theme(axis.title.x = element_text(colour = "black",
        ...
        axis.title.y = element_text(colour = "black",
                                   face = "bold")) +
  geom_label_repel(data = dplyr::filter(write_fit_list$female_PE, direction == 1 & logFC >= 1),
                  aes(label = label),
                  max.overlaps = Inf,
                  fill = "white",
                  show.legend = FALSE) +
...
# plot logFC density to check for approximate normality
ggplot(data = write_fit_list$BMI) +
  geom_density(aes(logFC))
...
The gene ranking method here is taken from Kat's lab (John I think).

```{r create GSEA input files}

# make the male files
# male_gsea <- write_fit_list$male_PE %>% 
#   dplyr::filter(., gene_biotype == "protein_coding")
# male_gsea <- male_gsea[ ! rownames(male_gsea) %in% removeDuplicates, ]
# male_gsea$Rank <- (sign(male_gsea$logFC)) * (male_gsea$p.value.adj)
# writexl::write_xlsx(as.data.frame(male_gsea) %>% 
#   tibble::rownames_to_column("ensembl") %>% 
#     dplyr::filter(., gene_biotype == "protein_coding") %>%
#     dplyr::select(., gene_name = ensembl, Rank) %>%
#     dplyr::arrange(desc(Rank)), "male_gsea.xlsx")

male_gsea <- allTable_male_PE %>% 
  dplyr::filter(., gene_biotype == "protein_coding")

male_gsea["Rank_log"] <- (sign(male_gsea$logFC)) * (-log(male_gsea$adj.P.Val,10))
# male_gsea["Rank_not_log"] <- (sign(male_gsea$logFC)) * -(male_gsea$adj.P.Val)
# male_gsea["Rank_not_log_div_1"] <- (sign(male_gsea$logFC)) * -(1/male_gsea$adj.P.Val)
...
# make the female files
# female_gsea <- write_fit_list$female_PE
# female_gsea$Rank <- (sign(female_gsea$logFC)) * (female_gsea$p.value.adj)
# writexl::write_xlsx(as.data.frame(female_gsea) %>% 
#   tibble::rownames_to_column("ensembl") %>% 
#   dplyr::select(., gene_name = ensembl, Rank) %>% 
#     dplyr::arrange(desc(Rank)), "male_gsea.xlsx")
...
temp <- data.frame(Z_log2CPM_counts[GOI,]) %>% 
  set_colnames(GOI) %>% 
  tibble::rownames_to_column("name_flowCell") %>% 
  left_join(., DGElist_mRNA_38$samples[, c("name_flowCell", "Outcome", "Sex")], by = "name_flowCell") %>% 
...
ggplot(melt_temp, aes(x = outcome_sex,
                y = value)) +
  stat_boxplot(geom = "errorbar",
               position = position_dodge(0.5),
               width = 0.2) +
