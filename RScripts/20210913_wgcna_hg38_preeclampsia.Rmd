...
vp_left <- viewport(x = 0, 
                    y = 0,
                    width = 0.5,
                    height = 1,
                    just = c(0,0))
vp_right <- viewport(x = 0.5,
                     y = 0,
                     width = 0.5,
                     height = 1,
                     just = c(0,0))
...
Import the sample metadata and ensure the metadata and counts are confluent.
...
# Y-chromosome Establish the DGEList object

This object is a list of three data.frames, `sample` (sample metadata with the samples as rows and traits as columns), `counts` (gene counts in cpm, where genes are rows and samples are columns), and `genes` (gene annotation from *AnnotationHub*, where genes are rows and gene information as columns).

```{r DGEList mRNA}

# # put the ensemble gene IDs into column 1
# y_Counts <- grch38_mRNA_placenta_counts[rownames(grch38_mRNA_placenta_counts) %in% onlyY$gene_name,]
# 
# # establish the DGEList
# y_DGElist_mRNA_38 <- DGEList(counts = y_Counts,
#                            samples = placenta_meta,
#                            genes = (y_Counts %>%
#                                       tibble::rownames_to_column())[,1,drop=FALSE])
# # set max digits
# options(digits=3)
# # take a look at the metadata
# # DGElist_mRNA_38$samples
# 
# # load the gene info library
# library(org.Hs.eg.db)
# y_DGElist_mRNA_38$genes$Symbol <- mapIds(org.Hs.eg.db,
#                                        keys = rownames(y_DGElist_mRNA_38),
#                                        keytype = "ENSEMBL",
#                                        column = "SYMBOL")
# # view head and tail of gene list
# head(y_DGElist_mRNA_38$genes)
# tail(y_DGElist_mRNA_38$genes)
# 
# # drop the 'genes' without official gene symbols
# y_DGElist_mRNA_38 <- y_DGElist_mRNA_38[!is.na(y_DGElist_mRNA_38$genes$Symbol), ]
# 
# # check the dimensions
# dim(y_DGElist_mRNA_38)
...
Here the filtering criteria is set to 1.5 CPM in `{r num_y_Filter}` samples - there are `{r num_y_Filter}` PE male samples.
Dropping the number of samples down will mean we keep the Y-chromsome genes that would otherwise have been lost if we used 15 samples as the cut-off.
Criteria for variable genes will remain at > than the bottom 40%.

```{r}

# num_y_Filter <- dim(dplyr::filter(y_DGElist_mRNA_38$samples, Sex == "M" & Outcome == "PE"))[2]
# # logical vector of genes meeting the filtering criteria
# y_keepTheseGenes <- rowSums(cpm(y_DGElist_mRNA_38) > 1.5) >= num_y_Filter
# 
# # logical vector of genes meeting the variability criteria
# y_variableGenes <- rowSds(cpm(y_DGElist_mRNA_38, log=TRUE)) > quantile(rowSds(cpm(y_DGElist_mRNA_38, log=TRUE)), 0.4)
# 
# # Perform the filtering step and recalculate the TMM normalisation factors for each library. 
# y_DGElist_mRNA_38 <- y_DGElist_mRNA_38[y_keepTheseGenes&y_variableGenes,,keep.lib.sizes = FALSE]


```

# Autosomes + X-chromosome Establish the DGEList object
...
# # put the ensemble gene IDs into column 1
# AX_Counts <- grch38_mRNA_placenta_counts[rownames(grch38_mRNA_placenta_counts) %notin% onlyY$gene_name,]
# 
# # establish the DGEList
# AX_DGElist_mRNA_38 <- DGEList(counts = AX_Counts,
#                            samples = placenta_meta,
#                            genes = (AX_Counts %>%
#                                       tibble::rownames_to_column())[,1,drop=FALSE])
# 
# # load the gene info library
# library(org.Hs.eg.db)
# AX_DGElist_mRNA_38$genes$Symbol <- mapIds(org.Hs.eg.db,
#                                        keys = rownames(AX_DGElist_mRNA_38),
#                                        keytype = "ENSEMBL",
#                                        column = "SYMBOL")
# # view head and tail of gene list
# head(AX_DGElist_mRNA_38$genes)
# tail(AX_DGElist_mRNA_38$genes)
# 
# # drop the 'genes' without official gene symbols
# AX_DGElist_mRNA_38 <- AX_DGElist_mRNA_38[!is.na(AX_DGElist_mRNA_38$genes$Symbol), ]
# 
# # check the dimensions
# dim(AX_DGElist_mRNA_38)
...
# # Retain genes which are expressed at sufficiently high levels, at least 1.5 cpm in samples with PE `numSamples`.
# numSamples <- dim(dplyr::filter(AX_DGElist_mRNA_38$samples, Outcome == "PE"))[2]
# keepTheseGenes <- rowSums(cpm(AX_DGElist_mRNA_38) > 1.5) >= numSamples
# 
# # Retain genes with sufficiently high variation in expression across all samples.
# # Here we define sufficiently high variation as having a standard deviation above the 40th quantile (so not in bottom 60%).
# variableGenes <- rowSds(cpm(AX_DGElist_mRNA_38, log=TRUE)) > quantile(rowSds(cpm(AX_DGElist_mRNA_38, log=TRUE)), 0.4)
# 
# # Perform the filtering step and recalculate the TMM normalisation factors for each library. 
# AX_DGElist_mRNA_38 <- AX_DGElist_mRNA_38[keepTheseGenes&variableGenes,,keep.lib.sizes = FALSE]

```

## Recombine the counts into a single DGEList object

To recombine the seperate counts we can simply rbind the y_chromosome counts to the others. However, we also need to update the library size column of the DGEList$samples table. To do this I will first join the two tables on the `name_flowCell` column, and then use `dplyr::mutate` to contstruct a new library size column prior to TMM normalisation.

```{r WGCNA-ReCombine-counts, echo=FALSE}

# # create a new DGEList object to hold all genes (first step is a copy of the larger of the two DGELists)
# DGElist_mRNA_38 <- AX_DGElist_mRNA_38
# 
# # rbid the y_chromsome counts to the AX_counts
# DGElist_mRNA_38$counts %<>% rbind(y_DGElist_mRNA_38$counts)
# 
# # left join the y_chromosome$sample to the new full DGEList - join only library size
# DGElist_mRNA_38$samples <- left_join(dplyr::select(AX_DGElist_mRNA_38$samples, lib.size_AX = lib.size, everything()), 
#                                      dplyr::select(y_DGElist_mRNA_38$samples, name_flowCell, lib.size_y = lib.size), by = "name_flowCell") %>% 
#   # mutate a new lib.size column by adding the other two library sizes together
#   dplyr::mutate(., lib.size = lib.size_AX+lib.size_y)
...
#   enricher(gene = deGenes$entrezid, 
#          TERM2GENE = x,
